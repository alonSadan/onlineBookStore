...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class<? extends Message>,Callback> callbacksByTypes;  //the key needs to be a type of message, not sure this is the correct way of writing it//
    ...
    public MicroService(String name) {
        this.name = name;
        this.callbacksByTypes = new ConcurrentHashMap<>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        callbacksByTypes.putIfAbsent(type,callback);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        ...
        return MessageBusImpl.getInstance().sendEvent(e); //maybe store the future in a table//
        //"non-Blocking", what is the meaning of this here?//
    }
    ...
    public final void run() {
      MessageBusImpl.getInstance().register(this);  //add a queue to the messageBus queuesByMicroservice//
        initialize();
        while (!terminated) {
            try {
                Message m = MessageBusImpl.getInstance().awaitMessage(this);
                callbacksByTypes.get(m.getClass()).call(m);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
        MessageBusImpl.getInstance().unregister(this);
        System.out.println(this.getName() + " reached the end of run()");
    }
